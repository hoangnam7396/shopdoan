...
class PageController extends Controller
{
    ...
    {
        /*$newproduct = Product::where('new',1)->take(12)->get();
        $topproduct = Product::where('promotion_price','<>',0)->take(8)->get();*/
        $newproduct = Product::where('new',1)->paginate(12);
    ...
    public function getSanPham($id)
    {
        ...
        $price_up=$product_price*(20/100);
//        dd($product_price,$price_up);
        $same_pro = Product::where('id_type',$product->id_type)->take(6)->get();
        $new_pro = Product::where('new',1)->take(4)->get();
        $best_seller = Product::where('promotion_price','>',0)->take(4)->get();
        $same_price=Product::where('unit_price','>=',$product_price-$price_up)
            ->where('unit_price','<=',$product_price+$price_up)
            ->inRandomOrder()
            ->limit(10)
            ->take(5)
            ->get();
//        dd($same_price);
    ...
    public function postSearch(Request $request)
    {
        ...
        $product = Product::where('name','like','%'.$request->tukhoa.'%')
            ->orWhere('description','like','%'.$request->tukhoa.'%')
            ->orWhere('content','like','%'.$request->tukhoa.'%')
            ->get();
